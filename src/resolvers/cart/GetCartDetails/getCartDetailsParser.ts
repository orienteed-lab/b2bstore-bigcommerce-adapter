import { GetCartDetailsQuery } from '@schema';

export const getCartDetailsParser = ({ data }: any): GetCartDetailsQuery => {
            ...
            prices: {
                ...
                applied_taxes: [
                    {
                        amount: {
                            currency: data.currency.code,
                            value: 8 // TODO_B2B: Find a way to get the tax value
                        }
                    }
                ],
                discounts: data.discounts
                    ? data.discounts.map((discount: any) => ({
                          ...
                          label: '' // TODO_B2B: No label name for discounts
                      }))
                    : null
            },
            applied_coupons: data.coupons
                ? data.coupons.map((coupon: any) => ({
                      code: coupon // TODO_B2B: Need to add a coupon to see its attributes
                  }))
                : null,
